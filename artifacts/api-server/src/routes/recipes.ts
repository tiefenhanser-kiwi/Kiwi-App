// /recipes/* — recipe import + scale routes.
//
// WS7-3 A1: the catalog GETs (GET /recipes, GET /recipes/:id) moved to
// createMealsRouter as GET /meals + GET /meals/:id. What remains here is the
// four import/scale POSTs. This module was converted from a plain Router() to
// the standard createRecipesRouter(deps?) factory in the same workstream,
// closing D-WS6-038 (the lone non-factory route holdout).
...
export function createRecipesRouter(
  ...
  // 35 MiB JSON ceiling: 5 images × 5 MiB raw × ~1.37 base64 overhead =
  // ~34.3 MiB of base64 plus JSON wrapper. Per-image and total decoded-byte
  // caps below enforce the real product limit (5 MiB / 25 MiB) after parsing.
    ...
    async (req, res) => {
      ...
      try {
        ...
        return res.json({
          success: false,
          reason:
            code === "invalid_url" || code === "blocked_host"
              ? "url_parse_failed"
              : "fetch_error",
      ...
      const source: "structured_data" | "ai_fallback" = jsonLd
        ? "structured_data"
        : "ai_fallback";
      ...
      // Validate the recipe content once more to surface any drift before
      // persistence. (The discriminated-union parse already covered this —
      // the explicit re-parse here is a no-op safety net while the schema
      // is new.)
      ...
      if (userId) {
        prisma.userActivity
          ...
          .catch((err) => {
            logger.warn(
              { err, url },
              "recipe_imported_url activity write failed",
            );
          });
      }
...
const router: IRouter = createRecipesRouter();
