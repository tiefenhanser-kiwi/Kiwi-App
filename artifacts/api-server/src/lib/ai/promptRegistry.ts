...
const REGISTRY: ReadonlyMap<string, PromptDescriptor> = new Map([
  ...
  // WS9 3c §7.6 — Surprise-me (zero-input) generate. BUG-039: the key was
  // seeded to the DB (prisma/seeds/aiPrompts.ts) but never added to this
  // runtime REGISTRY allowlist, so resolvePromptDescriptorFromDb threw
  // UnknownPromptKeyError → a bare 500 on every Surprise-me tap. (BUG-036 — the
  // mobile no-input guard — was a separate, correctly-fixed issue.)
  [
    "wizard.surprise.generate",
    {
      body: placeholder("wizard.surprise.generate"),
      defaultModel: MODEL_SONNET,
      defaultMode: "tool",
      toolDescription:
