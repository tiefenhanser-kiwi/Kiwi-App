...
const RETIRED_KEYS: readonly string[] = [
  ...
  // Row 5 · Block 1c (D-WS9-246) — the stock-image relevance judge went with
  // the stock providers: "AI for every image, no exceptions". Body, seed entry
  // and the registry entry are all deleted; this sweeps the row.
...
- **step.content names the ingredient AND carries its amount, taken from the ingredient list — never invented.** Reuse the list's numbers in the step as natural cooking measures, with fraction glyphs (½, ¼, ¾, ⅓, 1½) for non-whole amounts, never decimals (write "1½ cups", not "1.5 cups"); when the list has no quantity for an ingredient, name it without one ("season with salt and pepper", not "season with 1 tsp salt and 1 tsp pepper"); a part of a listed ingredient stays as written ("half the onion", "the remaining ¾ cup of broth"); temperatures, durations and doneness cues stay inline.
...
- \`preferencesContext.maxCookTimeMinutes\` (when set) → the shelf already honors it: a capped shelf is filtered on each meal's \`estimatedTimeMinutes\`, a start-to-plate time measured from that meal's steps, so every shelf meal fits the cap (under \`preferencesContext.maxCookTimeCoverage: "most"\`, at most one shelf row runs over it, by at most 20 minutes, and its \`estimatedTimeMinutes\` shows that). Trust a shelf meal's number. For a dinner you compose fresh, the cap limits what you may choose. The cap is a ceiling, not a target: every dinner must be on the table within it, from opening the fridge with prep included, and comfortably under is better than close to it. A 20-minute dinner is a good answer to a 30-minute limit, not a missed one — do not stretch a plan's dinners to fill the time allowed. Under a short cap that means few components, a quick-cooking protein and simple assembly. Anything that braises, roasts, bakes or simmers longer than the cap does not fit, however it is named — a slow-simmered stew is not a 30-minute dinner because its title fits on one line. If no fresh dinner honestly fits a slot, use a shelf meal. Cook time is independent of \`difficulty\`/\`weeklyPacing\`: a dish can be simple-but-slow (a hands-off braise) or involved-but-fast, so weigh the minute cap on its own, not as a proxy for fanciness.
...
- \`estimatedTimeMinutes\` — WALL-CLOCK minutes from opening the fridge to being able to plate, INCLUDING time the food cooks unattended. Required.
  ...
  - ⚠️ THIS NUMBER IS YOUR OWN ESTIMATE, AND IT IS NOT WHAT THE USER SEES. The app computes the time shown on the card from the \`outline\` you write for each dish (below), and recomputes it again at save from the written steps. Your number is kept only to check whether it agrees with your outline — so write the number the outline implies, not a number that looks good.
...
- \`dishes\` — array of one or more dishes that compose the meal. Most weeknight meals are a single dish (\`role: "main"\`); a fancier meal can include sides (\`role: "side"\`), sauces (\`role: "sauce"\`), or toppings (\`role: "topping"\`). A single-dish meal must itself be complete — a one-pot/one-pan dish carrying a protein AND a starch and/or vegetable (a soup, stew, chili, stir-fry, one-pot pasta, or hearty dinner salad), never a bare protein. Each dish has:
  ...
  - \`outline\` — the dish's timed step outline, \`[{ phaseType, estimatedMinutes, isTimingSensitive }]\`. See the outline rules below.
...
# The timed outline (no step text)

For each dish, also write \`outline\`: the recipe's steps in order as \`{ phaseType, estimatedMinutes, isTimingSensitive }\` — no step text. The written steps are generated by a separate, save-time call so the user only pays that latency for the plan they keep; do not include a \`steps\` field (the server drops it). The outline is what the app computes the meal's time from, so it must describe the recipe you actually chose, at the grain it will actually be written:

- Write the outline at the grain the written recipe will have — the same steps a home cook follows, in order, one entry per step: usually 4–10 per dish (1–3 for a genuinely simple side; do not pad). Begin with the prep steps as separate entries (chop, measure, mix a marinade — each its own step at 3–6 minutes), include a \`preheat\` wherever an oven, grill or pan must come to heat, a \`rest\` for anything that waits with no work — meat resting, a marinade or soak sitting, a chill — and end each dish with an \`assemble\` step for plating. A soak or marinade that sits is a \`rest\` step of its own, never folded into the prep before it. ⚠️ One prep entry standing in for all the knife work, or a dish with no plating entry, is a thinner outline than the recipe — and a time computed from it comes out short of the time the written steps will give.
...
**Name the ingredient AND carry its amount — taken from the ingredient list, never invented.** Like Kiwi's own recipes, a step names what goes in and says how much ("Stir in 2 cloves of minced garlic", "Pat 1½ pounds of shrimp dry and season with 1 tsp salt"): the ingredient list is the source of truth for quantities — reuse its numbers in the step as natural cooking measures, with fraction glyphs (½, ¼, ¾, ⅓, 1½) for non-whole amounts, never decimals (write "1½ cups", not "1.5 cups"). NEVER invent an amount the source did not state: when the list has no quantity for an ingredient, name it without one ("season with salt and pepper", not "season with 1 tsp salt and 1 tsp pepper"). When a step uses PART of a listed ingredient, say the part as written ("half the onion", "the remaining ¾ cup of broth", "1 tbsp of the butter"). Temperatures ("over medium-high heat", "at 425°F") and durations or doneness cues ("for 3-4 minutes", "until shimmering") stay inline — those are what a beginner needs on the step.
