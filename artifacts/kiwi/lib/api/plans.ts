...
export const PlanDetailSchema = z.object({
  ...
  sourceType: z.string(),
  // WS9-2 2c Commit 5 (D-WS9-144) — `image` REMOVED from the parse. Plan Review
  // no longer renders a header photo, and the server no longer emits the field.
  // (A plain z.object strips unknown keys, so an older server that still sends
  // it parses fine — this direction of the rollout is safe.)
