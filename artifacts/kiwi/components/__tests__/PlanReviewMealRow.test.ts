...
// row may act. That is `readOnly`. (It was born as WS9 3c's draft mode with an
// onReadOnlyEdit guard that EXPLAINED why editing was off; the draft surface and
// the guard prop left with D-WS9-191, so inert is now readOnly's only meaning.)
//
  ...
  await act(async () => {
    renderer = TestRenderer.create(
      React.createElement(PlanReviewMealRow, {
        ...
        planId: "plan-1",
        readOnly: true,
        // The composted wiring: readOnly, mutators still supplied.
